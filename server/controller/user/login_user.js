...
const login_user = (req, result) => {
    let email = req.body.email;
    let password = req.body.password;

    /* Check email enterred or not */
    if (!email) {
        return result.status(400).send(JSON.stringify({ "message": "Email is Required", "status": 400 }));
    ...
    if (!password) {
        ...
        return result.status(400).send(JSON.stringify({ "message": "Password is Required", "status": 400 }));
